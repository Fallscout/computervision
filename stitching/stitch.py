...
def im_stitch(img_left, img_right, naive=False, patchsize=3, xcorr=False):
    ...
    logging.info(f"Number of keypoints in left image: {len(kps_left)}")
    logging.info(f"Left features shape: {features_left.shape}")
    logging.info(f"Number of keypoints in right image: {len(kps_right)}")
    logging.info(f"Right features shape: {features_right.shape}")
    ...
    logging.info("Homography:\n{}".format(h))

    result = cv2.warpPerspective(
        img_right,
        h,
        (
            img_left.shape[1] + img_right.shape[1],
            max(img_left.shape[0], img_right.shape[0])
        )
    )
...
def extract_patches(img, keypoints, patchsize):
    ...
    padded_image = cv2.copyMakeBorder(img, border_height, border_height, border_width, border_width,
    ...
    for keypoint in keypoints:
        ...
        patches.append(
            padded_image[row1 + border_height:row2 + border_height, col1 + border_width:col2 + border_width].flatten())

    return np.array(patches)


def detect_corners(img, max_corners, quality_level, min_distance):
    result = cv2.goodFeaturesToTrack(img, max_corners, quality_level, min_distance)
    # Need to swap columns
    return result.reshape(result.shape[0], 2)[:, [1, 0]]


def ransac(matches, s, N, d, T):
    use_own_impl = False  # Own implementation does not work correctly
    if use_own_impl:
        ...
        for i in range(N):
            ...
            a1 = np.array(
                [[-x, -y, -1, 0, 0, 0, x * x_prime, y * x_prime, x_prime] for x, y, x_prime, y_prime in model_points])
            a2 = np.array(
                [[0, 0, 0, -x, -y, -1, x * y_prime, y * y_prime, y_prime] for x, y, x_prime, y_prime in model_points])
...
if __name__ == "__main__":
    ...
    stitched = im_stitch(cv2.imread(args.left), cv2.imread(args.right), naive=args.naive, xcorr=args.xcorr)
